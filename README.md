...
2. **قراءة ملف جهاز الفحص بترتيبه المطبوع:**
   - أجهزة Ediag / Launch تكتب صفحاتها من تحت لفوق، ومكتبة قراءة الـ PDF ترجّع النص بنفس ترتيب الملف — يعني مقلوب.
   - كاشف يعيد بناء الترتيب من إحداثيات كل سطر في الصفحة، فكل حالة عطل (`Current` / `Pending` / `History`) تقعد تحت الكود اللي تخصّه، موش تحت اللي قبله.
3. **محرك التحليل (Google Gemini):**
...
4. **لوحة الفيوز والفيشة والأفوميتر (Component & Fuse Locator):**
   - كل لوحة تبدأ بمصدر كلامها: `مرجع` (الكود موجود عندنا بالرقم والأمبير) ولا `إرشاد عام` (عائلة الكود بس). رقم فيوز أو أمبير أو علامة على الرسم ما يظهرش إلا لما يكون مقروء من المرجع — الكود المجهول ما يتخترعش له رقم.
   - المسار يتحدد من المنظومة اللي طلع منها الكود موش من حرفه الأول: عطل ستيرسو يوديك لعمود الدركسيون، وعطل سرعة عجلة يوديك للكوشينة، وعطل شبكة (CAN Time-Out) يوديك لخطوط البيانات — حتى لو الثلاثة أكواد `C`.
   - أكواد الإيرباق (`B`) تجي بتحذير قبل أي قراءة: ما تقيسش دائرة الكبسولة بالأفوميتر.
   - إرشادات الفحص العملي بالفولتية والمقاومة عبر جهاز الملتيميتر (Pinout & Multimeter Test).
5. **محرك بحث حي متعدد الطبقات لصور قطع الغيار:**
...
6. **أدوات التصدير والمشاركة المعتمدة:**
...
│   │   ├── pdf-parser.ts      # قراءة PDF جهاز الفحص بترتيب الصفحة المطبوعة
